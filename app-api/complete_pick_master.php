...
if (isset($_POST['pick_id'])) {
	...
	$nowtime=date("H.i");
			...
			$queryDelete="DELETE FROM picked_list_details WHERE pick_status='Not Confirmed' AND pick_id='".$_POST['pick_id']."'";
			$delqueryExc=mysql_query($queryDelete)or die('Error:'.mysql_error());
	 ...
	 if($queryUpdate)
	 {
  	...
  	$querycnt = "SELECT pd.pick_id,
						...
						pm.pick_completed_time	
						 FROM pick_list_master pm,pick_list_details pd  WHERE pm.pick_id='".$_POST['pick_id']."' AND pd.pick_id='".$_POST['pick_id']."' AND pm.pick_status='Completed' AND (pd.pick_status<>'Active' OR pd.pick_status<>'Not Picked') AND new_price_status='0' ORDER BY pd.Slno ASC ";
    $resouter = mysql_query($querycnt)or die('Error:'.mysql_error());
    $set = array();
    ...
    if ($total_records >= 1) {
		...
		$querycnt2 = "SELECT pd.pick_id,
						...
						pm.pick_assign_time,
						pm.pick_completed_time	
						 FROM pick_list_master pm,picked_list_details pd  WHERE pm.pick_id='".$_POST['pick_id']."' AND pd.pick_id='".$_POST['pick_id']."' AND pd.pick_status='Confirmed' ORDER BY pd.Slno ASC ";
    $resouter2 = mysql_query($querycnt2)or die('Error:'.mysql_error());
    $total_records2 = mysql_num_rows($resouter2);

    if ($total_records2 >= 1) {
            
		while ($results2 = mysql_fetch_array($resouter2, MYSQL_ASSOC)){
				//$response[$i] = $results;
			 	$response[$i] =array(
				"pick_id" => $results2['pick_id'],
				"partno"=> stripslashes($results2['partno']),
				"picked_qty"=> $results2['picked_qty'],     
				"picked_rate"=> $results2['picked_rate'],
				"bin_qrcode"=> $results2['bin_qrcode'],
				"picker_emp_code"=> $results2['picker_emp_code'],
				"pick_assign_time"=> $results2['pick_assign_time'],
				"pick_completed_time"=> $results2['pick_completed_time']);
