...
if (isset($_POST['company_id']) && isset($_POST['warehouse_id'])) {
  ...
  $querycnt = "SELECT gate_entry_master.grn_no,
					  ...
					  gate_entry_master.isTransferIn,
					  gate_entry_master.transferOut_status,
					...
					AND gate_entry_master.isTransferIn='1'
					AND gate_entry_master.transferOut_status='1'
                    AND DATE(gate_entry_master.gate_entry_datetime)=GETDATE() AS Date ) 
    ...
    if ($total_records >= 1) {
		...
		while ($results = mysql_fetch_array($resouter, MYSQL_ASSOC)){
				$response["TodayTransferOut"][$i] = $results;
		$i++;
		}
