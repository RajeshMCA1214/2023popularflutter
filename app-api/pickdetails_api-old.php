 ...
 $querycnt = "SELECT * FROM pick_list_master  WHERE  pick_assigned_status='1' AND (pick_category='".$_POST['category']."' OR pick_category='DIRECT')  AND picker_emp_code='".$_POST['employeeCode']."' AND  compcode='".$_POST['compCode']."' AND  pick_id='".$_POST['pickid']."'";
   ...
   $querycnt = "SELECT * FROM pick_list_master  WHERE  pick_assigned_status='1' AND pick_category='".$_POST['category']."' AND picker_emp_code='".$_POST['employeeCode']."' AND  compcode='".$_POST['compCode']."' AND  pick_id='".$_POST['pickid']."'";
    ...
    if ($total_records >= 1) {
		...
		if($_POST['category']=='PRO COURIER'){
		 $querycnt1 = "SELECT pick_list_details.*, (quantity-picked_qty) as pending_qty FROM pick_list_details WHERE (pick_category='".$_POST['category']."' OR pick_category='DIRECT')AND  pick_id='".$_POST['pickid']."' AND pick_status='Active' ORDER BY rackno,pick_status ASC LIMIT 0,5";
		 }
		 else{
		  $querycnt1 = "SELECT pick_list_details.*, (quantity-picked_qty) as pending_qty FROM pick_list_details WHERE pick_category='".$_POST['category']."' AND  pick_id='".$_POST['pickid']."' AND pick_status='Active' ORDER BY rackno,pick_status ASC LIMIT 0,5";
